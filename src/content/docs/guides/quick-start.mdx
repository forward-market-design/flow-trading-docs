...
<Aside type="note">
The rest of this guide assumes `ftdemo` is directly available in your system path. If you build from source, you will need to explicitly refer to the binary target, e.g. replace `ftdemo` with `./target/release/ftdemo`.
</Aside>
...
The available configuration options are available via	`ftdemo --help` and are discussed in the [project README](https://github.com/forward-market-design/flow-trading-service/blob/main/fts-demo/README.md#configuration). 
